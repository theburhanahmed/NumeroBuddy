...
export function CosmicTooltip({
  ...
  return <div className="relative inline-block">
      <div {...triggerProps} className="cursor-help" role="button" aria-label="Show help" tabIndex={0}>
        {icon ? <HelpCircleIcon className="w-5 h-5 text-cyan-400 hover:text-cyan-300 transition-colors" /> : children}
      </div>
      ...
      <AnimatePresence>
        ...
        duration: 0.2
      }} className={`absolute z-[9999] ${positionStyles[position]}`} style={{
        pointerEvents: 'none'
      }}>
