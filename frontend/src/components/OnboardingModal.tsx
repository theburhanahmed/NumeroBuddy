...
export function OnboardingModal() {
  ...
  return <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        opacity: 0
      }} className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={handleDismiss} />
        ...
        damping: 25
          ...
          <SpaceCard variant="premium" className="p-8 relative overflow-hidden">
            {/* Close Button */}
            <button onClick={handleDismiss} className="absolute top-4 right-4 p-2 rounded-xl bg-[#0a1628]/60 backdrop-blur-xl border border-cyan-500/20 text-white/70 hover:text-white hover:bg-[#0a1628]/80 transition-all">
            ...
            <div className="relative z-10 text-center">
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                ...
                icon: '🚀',
                label: 'Insightful'
              }].map((feature, i) => <motion.div key={feature.label} initial={{
                opacity: 0,
                y: 20
              }} animate={{
                opacity: 1,
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <SpaceButton variant="primary" size="lg" onClick={handleStart} icon={<ArrowRightIcon className="w-5 h-5" />} className="flex-1">
                  Get Started
                </SpaceButton>
                <SpaceButton variant="ghost" size="lg" onClick={handleDismiss} className="flex-1">
                  Maybe Later
                </SpaceButton>
              </div>
