...
export function SpaceCard({
  ...
  const variantStyles = {
    default: 'bg-[#1a2942]/40 backdrop-blur-xl border border-cyan-500/20 shadow-lg',
    premium: 'bg-[#1a2942]/60 backdrop-blur-2xl border border-cyan-500/30 shadow-xl shadow-cyan-500/10',
    interactive: 'bg-[#1a2942]/40 backdrop-blur-xl border border-cyan-500/20 shadow-lg cursor-pointer'
  };
  // Standardized hover styles
  const hoverStyles = hover ? 'hover:border-cyan-500/50 hover:shadow-cyan-500/20 hover:shadow-2xl' : '';
  ...
  return <motion.div className={`${baseStyles} ${variantStyles[variant]} ${hoverStyles} ${className}`} whileHover={onClick || hover ? {
