...
export function VideoExplainer() {
  ...
  const thumbnailUrl = 'https://via.placeholder.com/1280x720/1a2942/00d4ff?text=NumerAI+Explainer+Video';
  return <section className="relative py-20 px-4 md:px-6">
      <div className="max-w-5xl mx-auto">
        ...
        delay: 0.2
          ...
          <SpaceCard variant="premium" className="overflow-hidden p-0">
            ...
            <div className="relative w-full h-full group cursor-pointer" onClick={() => setIsPlaying(true)}>
                  {/* Thumbnail image */}
                  <img src={thumbnailUrl} alt="NumerAI explainer video thumbnail" className="w-full h-full object-cover" />
            ...
            <div className="relative w-full h-full">
                  <iframe src={`${videoUrl}?autoplay=1`} title="NumerAI Explainer Video" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen className="w-full h-full" />

                  {/* Close button */}
                  <button onClick={() => setIsPlaying(false)} className="absolute top-4 right-4 w-10 h-10 rounded-full bg-black/80 backdrop-blur-sm flex items-center justify-center text-white hover:bg-black transition-colors z-10" aria-label="Close video">
        ...
        }].map((highlight, index) => <motion.div key={highlight.time} initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
