...
export function LiquidMetalOrb({
  ...
  return <motion.div className={`relative ${sizeMap[size]} ${className}`} animate={{
    y: [0, -20, 0],
    rotateY: [0, 360]
  }} transition={{
    y: {
    ...
    perspective: '1000px'
      ...
      transformStyle: 'preserve-3d'
        ...
        {/* Chrome Reflection */}
        <motion.div className="absolute inset-0 rounded-full overflow-hidden" animate={{
        rotate: [0, 360]
      }} transition={{
        duration: 8,
        repeat: Infinity,
        ...
        {/* Specular Highlight */}
        <div className="absolute top-[15%] left-[25%] w-[40%] h-[40%] rounded-full" style={{
        ...
        {/* Refraction Edge */}
        <div className="absolute inset-0 rounded-full" style={{
        background: 'radial-gradient(circle at 70% 70%, transparent 50%, rgba(67, 56, 202, 0.6) 80%, transparent 100%)',
        filter: 'blur(15px)'
      }} />
      </div>
