...
export function FloatingNeonRunes({
  count = 8
}: {
  count?: number;
}) {
  ...
  return <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {[...Array(count)].map((_, i) => <motion.div key={i} className="absolute" style={{
...
export function ParticleSwarm({
  ...
  return <div className="absolute inset-0 pointer-events-none overflow-hidden">
      ...
      return <motion.div key={i} className="absolute rounded-full" style={{
        ...
        x: [0, pathRadius * Math.cos(i / count * Math.PI * 2), pathRadius * Math.cos(i / count * Math.PI * 2 + Math.PI), 0],
        y: [0, pathRadius * Math.sin(i / count * Math.PI * 2), pathRadius * Math.sin(i / count * Math.PI * 2 + Math.PI), 0],
        opacity: [0, 1, 1, 0],
        scale: [0, 1, 1, 0]
      }} transition={{
        duration: duration,
...
export function NebulaStreaks() {
  return <div className="absolute inset-0 pointer-events-none overflow-hidden">
      {/* Horizontal Streaks */}
      {[...Array(5)].map((_, i) => <motion.div key={`h-${i}`} className="absolute h-px" style={{
      left: 0,
      right: 0,
      top: `${20 + i * 15}%`,
      ...
      {/* Diagonal Streaks */}
      {[...Array(3)].map((_, i) => <motion.div key={`d-${i}`} className="absolute w-full h-px origin-left" style={{
      left: 0,
      top: `${30 + i * 20}%`,
      background: `linear-gradient(90deg, 
              transparent 0%, 
      ...
      {[...Array(4)].map((_, i) => <motion.div key={`fog-${i}`} className="absolute rounded-full" style={{
