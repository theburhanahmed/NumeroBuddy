    ...
    <div className="grid md:grid-cols-3 gap-6">
      ...
      <motion.div
        ...
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 z-10">
          ...
          <MagneticCard
          variant={selectedTier === tier.id ? 'liquid-premium' : 'liquid'}
          className={`card-padding h-full ${selectedTier === tier.id ? 'ring-2 ring-purple-500' : ''} ${tier.recommended ? 'scale-105' : ''}`}>

            <div className="liquid-glass-content flex flex-col h-full">
              {/* Header */}
              <div className="text-center mb-6">
                <div
                className={`w-16 h-16 bg-gradient-to-br ${tier.color} rounded-2xl flex items-center justify-center text-white mx-auto mb-4 shadow-lg`}>
              ...
              {/* Limits */}
              {tier.limits &&
            <div className="mb-6 p-3 bg-gradient-to-br from-blue-500/10 to-purple-500/10 rounded-xl">
              ...
              {showSelection &&
            <GlassButton
              variant={tier.recommended ? 'liquid' : 'secondary'}
              size="lg"
              onClick={() => onSelectTier(tier.id)}
              className={`w-full ${tier.recommended ? 'glass-glow' : ''}`}
