...
function FeatureCell({
  ...
  if (typeof value === 'boolean') {
    return value ? <CheckIcon className="w-5 h-5 text-green-400 mx-auto" /> : <XIcon className="w-5 h-5 text-white/30 mx-auto" />;
  }
          ...
          <SpaceCard variant="premium" className="overflow-hidden">
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {features.map((feature, index) => <motion.tr key={feature.name} initial={{
                  opacity: 0
                }} whileInView={{
                  opacity: 1
                }} viewport={{
            ...
            <div className="md:hidden space-y-6 p-6">
              {['Free', 'Premium', 'Enterprise'].map(plan => <div key={plan} className={`p-6 rounded-2xl border ${plan === 'Premium' ? 'border-cyan-500/50 bg-cyan-500/5' : 'border-cyan-500/20'}`}>
                  <h3 className="text-xl font-bold text-white mb-4">{plan}</h3>
                  <div className="space-y-3">
                    {features.map(feature => {
                  const value = plan === 'Free' ? feature.free : plan === 'Premium' ? feature.premium : feature.enterprise;
                  return <div key={feature.name} className="flex items-center justify-between gap-4">
