...
export function Premium3DPlanet({
  ...
  return <div className={`relative ${sizeMap[size]} ${className}`} style={{
    perspective: '2000px'
  }}>
      {/* Main Planet */}
      ...
      transformStyle: 'preserve-3d'
        ...
        boxShadow: `
          ...
          background: 'radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.6) 0%, rgba(255, 255, 255, 0.3) 10%, transparent 25%)',
          filter: 'blur(8px)'
        }} animate={{
          opacity: [0.6, 0.8, 0.6]
        }} transition={{
      ...
      {/* Orbiting Moons */}
      {withMoons && <>
          {[1, 2, 3].map((moon, index) => <motion.div key={moon} className="absolute" style={{
        width: '100%',
        height: '100%'
      }} animate={{
        rotate: [0, 360]
      ...
      background: 'radial-gradient(circle, transparent 60%, rgba(11, 15, 25, 0.3) 90%, rgba(11, 15, 25, 0.6) 100%)',
      filter: 'blur(4px)'
    }} />
    </div>;
}