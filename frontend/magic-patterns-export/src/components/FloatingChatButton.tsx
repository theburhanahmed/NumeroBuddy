...
export function FloatingChatButton() {
  ...
  // Only show on dashboard/authenticated pages, not marketing pages
  const isDashboardPage =
  location.pathname.startsWith('/dashboard') ||
  location.pathname.startsWith('/life-path') ||
  location.pathname.startsWith('/compatibility') ||
  location.pathname.startsWith('/birth-chart') ||
  location.pathname.startsWith('/daily-readings') ||
  location.pathname.startsWith('/forecasts') ||
  location.pathname.startsWith('/settings') ||
  location.pathname.startsWith('/report') ||
  location.pathname.startsWith('/remedies') ||
  ...
  return (
    <motion.button
      onClick={openChat}
      className="fixed bottom-6 right-6 z-40 w-16 h-16 bg-gradient-to-r from-cyan-500 to-blue-600 rounded-full shadow-2xl flex items-center justify-center group"
      initial={{
        scale: 0,
        opacity: 0
      }}
