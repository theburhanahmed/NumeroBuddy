...
export function MobileNavDrawer({
  ...
  return <>
      {/* Trigger Button */}
      <button onClick={() => setIsOpen(true)} className="p-2 rounded-xl bg-[#1a2942]/60 backdrop-blur-xl border border-cyan-500/20 text-white" aria-label="Open navigation menu">
      ...
      <AnimatePresence>
        {isOpen && <>
          ...
          opacity: 0
        }} onClick={() => setIsOpen(false)} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100]" />
          ...
          stiffness: 300
              ...
              <div className="sticky top-0 bg-[#0B0F19] border-b border-cyan-500/20 p-4 flex justify-between items-center">
                ...
                <button onClick={() => setIsOpen(false)} className="p-2 rounded-xl bg-[#1a2942]/60 backdrop-blur-xl border border-cyan-500/20 text-white" aria-label="Close menu">
