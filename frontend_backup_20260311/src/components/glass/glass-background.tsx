...
/**
 * Deterministic seeded random for consistent server/client hydration
 */
...
/** Round to 4 decimals and format as string so server and client serialize identically (avoids hydration mismatch) */
...
export function GlassBackground({
  ...
  const stars = useMemo(() => {
    return [...Array(starCount)].map((_, i) => ({
      left: `${toFixed4(seededRandom(i * 3) * 100)}%`,
      top: `${toFixed4(seededRandom(i * 5 + 1) * 100)}%`,
      animationDelay: `${toFixed4(seededRandom(i * 7 + 2) * 3)}s`,
      opacity: toFixed4(seededRandom(i * 11 + 3) * 0.7 + 0.3),
    }))
  }, [starCount])
